...
Realistic sample estate documents for trying the upload → parse → alert pipeline.
Drag any of these into the Documents screen to watch the parser extract structured
facts and the DeadlineAgent react. The `demo-uploads/` subfolder holds a smaller
set themed to the seeded demo estate (Robert Milligan), plus the script that
generated them.
